...
/// streaming state, pending injections, and user messages.
struct TurnGate {
    ...
    /// User text queued while a turn was already running. Always takes priority
    /// over auto-turn chaining.
    queued_user_message: Option<String>,
    /// Consecutive auto-turns chained since the last user-driven turn.
    auto_depth: u32,
    /// Set by `interrupt_handler`; prevents `finalize_turn` from chaining
    /// another auto-turn — the gate is released instead, giving the user control.
...
fn spawn_turn(state: Arc<AppState>, trigger: TurnTrigger, cancel: CancellationToken) {
    // Gate must already be claimed (streaming=true) before calling this.
    tokio::spawn(async move {
...
/// End-of-turn handoff under the unified `TurnGate`. Drains any queued
...
fn finalize_turn(state: Arc<AppState>) {
    // Drain queued state under the gate, decide what happens next, and mint the
    // next turn's cancel token in the same locked block when we chain — so the
    // moment we release the lock the gate already holds the upcoming turn's
    // token. An interrupt racing the finalize→spawn window then cancels that
    // token (and `send_message`'s top-of-loop check honours it) instead of
    // harmlessly cancelling the just-finished turn's token.
    let (injections, queued_user, should_auto, next) = {
        let mut gate = state.turn_gate.lock().unwrap();
        ...
        let should_auto = !gate.interrupt_requested
            ...
            && queued_user.is_none()
            && !injections.is_empty();
        let next = if queued_user.is_some() {
        ...
        (injections, queued_user, should_auto, next)
    };
...
/// an auto-turn so the model reacts. No-op when nothing is queued. Uses the
/// unified `TurnGate` so that a queued user message always takes priority.
fn try_continue_auto(state: Arc<AppState>) {
    let mut gate = state.turn_gate.lock().unwrap();
    if gate.pending_injections.is_empty() { return; }
    if gate.streaming { return; }
    if gate.queued_user_message.is_some() { return; }
    if gate.interrupt_requested { return; }

    gate.streaming = true;
    gate.auto_depth = 1;
    // Mint the turn's cancel token under the gate lock before releasing it.
    let cancel = gate.begin_turn();
...
async fn handle_client_frame(raw: &str, state: &Arc<AppState>) {
    ...
    match frame_type {
        "user_message" => {
            ...
            let mut gate = state.turn_gate.lock().unwrap();
            if gate.streaming {
                // Queue instead of reject. The user message will be processed
                // after the current turn finishes (priority over auto-turns).
                if gate.queued_user_message.is_some() {
                    let json = serde_json::json!({"type":"error","message":"a message is already queued"}).to_string();
                    drop(gate);
                    buffer_and_fanout(&state.stream_state, json);
                    return;
                }
                let preview: String = text.chars().take(120).collect();
                gate.queued_user_message = Some(text);
                drop(gate);
                info!("[lair/stream] user_message queued (turn running): {preview}");
            ...
            gate.streaming = true;
            gate.auto_depth = 0;
            // Install the turn's cancel token under the same lock.
            let cancel = gate.begin_turn();
            drop(gate);
            let preview: String = text.chars().take(120).collect();
...
- **`create_agent`** — args: `git_url?`, `name?`, `port?`, `startup_prompt?`. Spawns a new *local* child process.
  - Omit `name` to have one auto-generated by a short model call from the spawn context (git_url + startup_prompt); pass `name` to force an exact name. If model naming fails the default is `lair-<repo-slug>`, or `lair-workload` with no `git_url`. `git_url` is a spawn-time argument only — it isn't persisted in the registry. The cloned repo lives in the agent's workspace dir and survives restarts.
...
- The lair container's SSH keypair is at `$HOME/.ssh/id_ed25519` (standard OpenSSH location). `mint_bootstrap_userdata` always embeds the matching pubkey in the userdata it returns.
    ...
    AnthropicTool {
        ...
        input_schema: serde_json::json!({
            ...
            "properties": {
                ...
                "name": {
                    "type": "string",
                    "description": "Optional exact name. If omitted, a short, memorable name is generated by a model call from the spawn context (git_url + startup_prompt); if that fails it falls back to lair-<repo-name>, or lair-workload with no git_url."
                },
                "port": {
                    "type": "integer",
...
async fn exec_create_agent_for_parent(
    ...
    // Deterministic fallback name, used only when no explicit name is given AND
    // model-based naming fails: `lair-<repo-slug>`, or `lair-workload`.
    let fallback_name = || match &git_url {
        Some(u) => {
    ...
    let (child_name, child_slug) = match input.get("name").and_then(|v| v.as_str()) {
        ...
        None => {
            ...
            let mut n = 2;
            while reg.get(&candidate).is_some() {
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            if exited || !matches!(status, Some(TaskStatus::Running)) {
                ...
                info!("[lair/monitor] {task_id} finished ({status_str}), stopping");
...
pub async fn run(print_pubkey: bool) -> anyhow::Result<()> {
    ...
    // conventional `$HOME/.ssh/id_ed25519` location so tool calls (`ssh
    // user@host`, `git push`, the agentic loop's `bash` shell-outs) pick
    // it up without `-i` flags. Lair seeds each spawned agent's
    // `$HOME/.ssh/` from this same keypair (see `AgentSupervisor::spawn`),
    // so the whole container shares one identity.
    let lair_home = std::env::var("HOME")
        .ok().filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| dir.clone());
    match ensure_container_ssh_keypair(&lair_home) {
