...
|---|---|---|
| Model sees output | once, on completion | repeatedly, while running + once on exit |
| Triggers a model turn | once (`bg_complete`) | one per wake-up with new output (`bg_progress`) + one final (`bg_complete`) |
...
(`TaskRecord`) and a bounded live output buffer (`TaskOutput`). A task created by
`monitor_process` additionally has a **monitor**: a detached loop that owns the task's
whole lifecycle — periodically waking the model with new output while it runs, and
once more when it exits.
...
`monitor_process` always takes a `command` — it starts that command as its own
background task and watches it. There is no "attach to an existing task" mode: a
monitor owns the task it watches. It also accepts an optional `wake_interval_secs`
and `purpose`. "Remote" monitoring is just the command reaching out — e.g.
`ssh host 'journalctl -f'`; the tool has no special remote mode.
...
`exec_monitor_process` mints a `task_id`, registers a `Running` `TaskRecord` (with
`wake_interval_secs` set), then starts two detached pieces sharing one `Notify`:

- `run_monitored_command` runs the `bash -c` child via `spawn_background_command`.
  On exit it finalizes the registry row and fires a push — but, unlike
  `run_tracked_command`, it does **not** wake the model itself. It just signals the
  shared `Notify` so the monitor loop delivers the single final wake-up.
- `spawn_monitor` is the loop. Each iteration waits on whichever fires first —
  `wake_interval_secs` elapsing, the `Notify` (process exited), or the cancel token —
  then:
  1. reads new output from the `TaskOutput` buffer since its last cursor;
  2. if the process has exited, queues one `bg_complete` injection with the final
     status + output, fans out a `bg_complete` event, calls `try_continue_auto`, and
     stops;
  3. otherwise, if there's new output, queues a `bg_progress` injection, fans out a
     `bg_progress` event, and calls `try_continue_auto`;
  4. on cancellation, stops silently — no wake-up.

A mid-run wake-up with no new output is skipped — the model is not woken for nothing —
but the final exit wake-up always fires. The monitor is the *only* thing that wakes
the model for its task, so there's exactly one task row and one completion event.
...
core/src/background.rs
  ...
  monitor_complete_text / _message — bg_complete final wake prompt
core/src/app.rs
...
lair/src/agent.rs, lair/src/lair.rs   (duplicated per role)
  ...
  run_tracked_command              — run_command_in_background: spawn + completion wake
  run_monitored_command            — monitor_process: spawn + finalize, no wake (loop owns it)
...
- **A monitor owns its task.** `monitor_process` always starts its own `bash -c`
  child; it can't attach to a task started by `run_command_in_background`. The
  registry shows one row per monitor.
