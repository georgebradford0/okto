...
octo is post-Docker — every "container env var" intuition from the old
...
|---|---|---|---|
| `~/.octo/config.json` | JSON | Credentials + model (`anthropic_api_key`, `openai_api_key`, `model`, `api_url`) | `octo config set` (or `octo init --…`) |
| `~/.octo/lair-env` | `KEY=VALUE\n` lines | Operator-supplied process env (`GH_TOKEN`, `BRAVE_API_KEY`, …) | `octo env set/unset KEY=VALUE` |
| `mcp.json` (per role) | JSON array | Per-MCP-server `env: {}` map | `octo mcp add --env K=V` |

Their lifetimes differ:

- **`config.json`** is read at every model call by `octo_core::resolve_api_key()` / `resolve_model()`. Rotation is **live**: edit it with `octo config set` and the next LLM request picks up the new value. No lair restart.
- **`lair-env`** is read once when lair spawns. `octo env set/unset` rewrites the file and **automatically restarts lair** to apply the change.
- **`mcp.json`** is watched by a 2-second mtime poller (`core/src/mcp.rs::start_mcp_watcher`). A change adds / removes MCP server processes without touching the parent role. `octo mcp add` waits up to 60 s for the new server's `[mcp] '<name>' connected` log line, rolls back on failure.
...
```
OCTO_DATA_DIR=$HOME/.octo/lair
OCTO_AGENTS_DIR=$HOME/.octo/agents
...
*managed* keys (`OCTO_DATA_DIR`, `OCTO_AGENTS_DIR`, `NOISE_PORT`,
`PUBLIC_PORT`, `OCTO_SKIP_SHELL_ENV`, `OCTO_LAIR_BINARY`) is hard-coded
in `cli/src/init.rs::MANAGED_ENV_KEYS`; `octo env set` rejects attempts to
override them.

Lair inherits the CLI's process env (which includes the operator's shell
env) by default. The `OCTO_SKIP_SHELL_ENV=1` flag tells
`octo_core::init_shell_env()` to *not* re-source `~/.zshrc` / `~/.bashrc` —
...
When the LLM calls `create_agent` (or `octo agents start` re-spawns a
stopped one), `lair/src/agent_proc.rs::spawn` sets on the child:

```
OCTO_DATA_DIR=$HOME/.octo/agents/<name>/data
WORKSPACE_DIR=$HOME/.octo/agents/<name>/workspace
...
you set with `octo env set FOO=bar` flows down to MCP servers spawned
...
OCTO_DATA_DIR=/var/lib/octo/data
WORKSPACE_DIR=/var/lib/octo/workspace
NOISE_KEY_FILE=/var/lib/octo/data/noise_key.bin
OCTO_SKIP_SHELL_ENV=1
AGENT_PURPOSE=…                 # optional
STARTUP_SCRIPT=…                # optional
```

Loaded by systemd via `EnvironmentFile=` in `/etc/systemd/system/octo-agent.service`.
...
2. `register_remote_agent` SSHes in (using lair's operator key) and drops
   ...
   `/var/lib/octo/data/config.json` over the SSH channel.
3. `systemctl restart octo-agent` so the agent re-reads it.
...
2. **`~/.octo/config.json`** (read live on every call via `read_config()`).
3. **`~/.zshrc` / `~/.bash_profile` / etc.** — only consulted when
   `OCTO_SKIP_SHELL_ENV` is *not* set. Effectively never used in
...
When `octo mcp add --env KEY=VALUE` runs, any `${VAR}` references in
...
```
$ octo mcp add --name brave --command npx --env BRAVE_API_KEY='${BRAVE_KEY}' -- @modelcontextprotocol/server-brave-search
...
octo config set --anthropic-api-key sk-ant-XXXX

# Switch to an OpenAI-compatible backend
octo config set --api-url https://api.openai.com/v1/chat/completions \
                --openai-api-key sk-XXXX --model gpt-4o

# GitHub token (lair restart)
octo env set GH_TOKEN=ghp_XXXX

# Brave search (turns on the `web_search` built-in tool — lair restart)
octo env set BRAVE_API_KEY=BSA-XXXX

# Add an MCP server with env (hot-reload)
octo mcp add --name linear --command npx \
  --env LINEAR_API_KEY=lin_api_XXXX \
  -- -y @linear/mcp-server

# Set a custom env on a specific child agent's MCP only
octo mcp add --agent lair-myrepo --name foo --command npx \
  --env CUSTOM_VAR='${HOST_SHELL_VAR}' -- @scope/foo

# Inspect the current state
octo config show                            # mask credentials, show model + url
octo env show                               # operator vars in ~/.octo/lair-env (masked)
octo mcp list --agent lair                  # MCP servers + their env keys
cat ~/.octo/config.json                     # raw
...
| `OCTO_DATA_DIR` | CLI (lair) / lair (children) | `octo_core::data_dir()` |
| `OCTO_AGENTS_DIR` | CLI (lair only) | `AgentSupervisor::new` |
| `OCTO_LAIR_BINARY` | CLI (lair only) | `AgentSupervisor` for child re-exec |
...
| `OCTO_SKIP_SHELL_ENV` | All managed roles | `init_shell_env()` short-circuit |
| `OCTO_DEV` | `./start_dev.sh` only | Pins dev Noise keypair + sets `PUBLIC_HOST=127.0.0.1` |
| `OCTO_RELAY_URL` | Operator (rare) | Override push-notification relay endpoint |
...
- **No more bind-mounted `config.json` at `/data/config.json`.** Config now lives at the same path on disk that every role reads (`~/.octo/config.json`); `octo_core::config_dir()` always resolves there regardless of `OCTO_DATA_DIR`, so lair / agents / CLI share it without inode-pinning gymnastics.
- **`docker inspect lair` leak surface is gone.** Operator env still goes to lair's process, but it only surfaces in `ps eww $(cat ~/.octo/lair/lair.pid)` for users with shell access. Same trust boundary, just one fewer way for it to leak.
- **`OCTO_AGENT_IMAGE` is retired.** Children re-exec the same binary as lair (`OCTO_LAIR_BINARY` resolves at lair startup).
