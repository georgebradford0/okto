...
Tools baked into the image (besides `lair`): `buildah`, `fuse-overlayfs`, `gcc`, `git`, `gh`, `glab`, `jq`, `node`/`npm`, `openssh-client`, `procps` (`ps`/`top`/`free`), `qrencode`, `slirp4netns`, `uidmap`, `unzip`, `uv`/`uvx`. (The AWS CLI is **not** baked in — operators who need it install it via `~/.okto/bootstrap.sh`.) `/etc/subuid` + `/etc/subgid` are populated for every agent uid (10001, 10100..10199) so each can run `buildah` rootless; the image defaults to the `vfs` storage driver (configured via `/etc/containers/storage.conf`).
...
| `ios.yml` | Builds on macOS runner, runs the Maestro `smoke.yaml` e2e flow on a simulator (gate), then optionally uploads to TestFlight. The `distribute` job `needs: build`, so a failing flow blocks the upload. |
