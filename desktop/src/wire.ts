// Wire schema for the /stream WebSocket between desktop and lair/agent.
//
...
//
// Kept byte-identical to mobile/src/wire.ts so the two clients can share
// rendering helpers later. If you tweak one, tweak the other.
//
...
// advertises its own version on `/info` and in every `ready` frame. Bump only
...
/** A child agent surfaced to mobile by lair's `agents` event. The desktop
 *  client reaches any agent's chat via `ws://<lair-tunnel>/agents/<id>/stream`
 *  — there is no direct port/pubkey/host for an agent because lair proxies all
 *  traffic. `kind` is `"local"` or `"remote"`; advisory, the proxy URL is the
 *  same. */
