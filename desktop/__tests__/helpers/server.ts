...
//
// Both are installed as globals by jest.setup.cjs. Tests drive the server side
...
//
// Kept deliberately close to mobile/__tests__/helpers/server.ts so the two
// behavioural suites read the same way.

/** The fixed loopback port the mocked `noise_connect` Tauri command returns;
 *  the renderer opens its WebSockets against `ws://127.0.0.1:<TUNNEL_PORT>`. */
export const TUNNEL_PORT = 45678
...
/** The socket opened for a given proxy sub-path (e.g. `/stream`, or an agent's
 *  `/agents/<id>/stream`). Defaults to the master `/stream`. */
export const wsFor = (suffix = '/stream'): FakeWebSocket | undefined =>
  [...FakeWebSocket.instances].reverse().find(w => w.url.endsWith(suffix))
